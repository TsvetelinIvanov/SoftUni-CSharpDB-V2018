...
AS 
  (SELECT ovm.ReturnOfficeId, ovm.OfficeId, ovm.Id, ovm.Manufacturer, ovm.Model
   FROM	(SELECT DENSE_RANK() OVER(PARTITION BY v.Id ORDER BY o.CollectionDate DESC) AS LatestRentCarsRank,
...
SELECT CONCAT(Manufacturer, ' - ', Model) AS Vehicle, [Location] = 
	      CASE
		...
		WHEN(cte_Rancs.OfficeId <> cte_Rancs.ReturnOfficeId) THEN (SELECT CONCAT(t.[Name], ' - ', o.[Name])
								     FROM Towns AS t
