--The query below causes a compile time error in Jugge, but but in database work correctly!!!
CREATE FUNCTION udf_GetReportsCount(@employeeId INT, @statusId INT) 
RETURNS INT AS
BEGIN
 DECLARE @reportsCount INT

 SET @reportsCount = (SELECT COUNT(r.StatusId)
                      FROM Employees AS e
...
END
--The query above causes a compile time error in Jugge, but but in database work correctly!!!
